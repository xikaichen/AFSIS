...
class Downloader(object):
    ...
    def make_query(self, params, starts=None, nrows=None):
        ...
        for k, v in params.items():
            ...
            #print self.base_url + query + limit
    ...
    def run_one_batch(self, starts, nrows):
        '''
        return True if no further batch, else False
        '''
    ...
    def run_all(self):
        start = 0
        nrows = 100
        print  'one batch starting...'
        while not self.run_one_batch(start, nrows):
            start += nrows
        print int(threadNum), 'completed, runs:', start / nrows


def main(config):

    d1 = Downloader(config)
    d1.run_all()

    endtime = datetime.datetime.now()
    print "The program end at:", endtime.strftime('%Y-%m-%d %H:%M:%S')
...
if __name__ == '__main__':
    ...
    print "The program start at:", starttime.strftime('%Y-%m-%d %H:%M:%S')
